...
/**
 * Myriad's representation of chess moves, each chess move consists of a
 * starting square an ending square and an appropriate modifier. Once a Move
 * object has been instantiated it cannot be changed!
 * 
 * @author Jesse Wang
 */
public final class Move {
	...
	public static final Move[] CASTLE = { WHITE_K_SIDE_CASTLING,
			BLACK_K_SIDE_CASTLING, WHITE_Q_SIDE_CASTLING, BLACK_Q_SIDE_CASTLING };

	// ----------------------End of Constants----------------------

	// ----------------------Constructors----------------------
	/**
	 * Makes a special move with 0 as it's starting square and destination as
	 * well as a specified modifier.
	 * 
	 * @param modifiers
	 *            : 1 = white king side castling, 2 = black king side castling,
	 ...
	 *            The ending square.
	 * @param modifier
	 *            Modifiers: 0 = no modifier, 5 = en passant, 6 = promote to
	 *            rook, 7 = promote to knight, 8 = promote to bishop, 9 =
	 *            promote to queen.
	 */
	...
	public boolean isEqual(Move m) {
		return m.getStartSquare() == start_sq && m.getEndSquare() == end_sq
				&& m.getModifier() == modifiers;
	}
	 ...
	 * 
	 * @param p
	 *            The current position, used to convert into expanded algebraic
	 *            notation.
	...
	public static Move toMove(String m_s) {
		if (m_s.equals("O-O (w)"))
			return WHITE_K_SIDE_CASTLING;
		else if (m_s.equals("O-O (b)"))
			return BLACK_K_SIDE_CASTLING;
		else if (m_s.equals("O-O-O (w)"))
			return WHITE_Q_SIDE_CASTLING;
		else if (m_s.equals("O-O-O (b)"))
			return BLACK_Q_SIDE_CASTLING;
		if (m_s.length() == 5)
			return new Move(stringTo0x88(m_s.substring(0, 2)),
					stringTo0x88(m_s.substring(3)));
		else if (m_s.length() == 6)
			return new Move(stringTo0x88(m_s.substring(1, 3)),
					stringTo0x88(m_s.substring(4)));
			...
			if (s == 'R')
				...
				md = 9;
			return new Move(stringTo0x88(m_s.substring(0, 2)),
					stringTo0x88(m_s.substring(3, 5)), md);
		} else if (m_s.length() == 9)
			return new Move(stringTo0x88(m_s.substring(0, 2)),
					stringTo0x88(m_s.substring(3, 5)), (byte) 5);
		else
			return new Move((byte) 0, (byte) 0);
	}
