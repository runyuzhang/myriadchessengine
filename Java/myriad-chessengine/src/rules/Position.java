...
public final class Position {
			...
			for (Piece current_piece : current_map) {
				...
				if (assailant.getType() == Piece.NULL) {
					...
					case Piece.KING:
						...
						int ind = getIndiceOfPiece(current_piece,
								is_White_to_Move ? true : false);
						for (int i = 0; i < 4; i++) {
							boolean can_castle = castle_rights[i];
							int diff = i < 2 ? RIGHT_MOVE : LEFT_MOVE, range = i < 2 ? 2
									: 3;
							next_pos = c_pos;
					...
					if (type == Piece.QUEEN)
						...
						pieceMoves.add(new Move(c_pos, next_pos));
							...
							&& (Math.abs(a_row - g_row) == Math.abs(a_col
									- g_col)))
						pieceMoves.add(new Move(c_pos, next_pos));
					else if (type == Piece.PAWN && (a_row - g_row) * c_col > 0
	...
	public Position makeMove(Move m) {
		Piece[] map = Arrays.copyOf(is_White_to_Move ? white_map : black_map,
				white_map.length);
		...
		if (modifier == 1) {
			cstl_rights[0] = false;
			cstl_rights[2] = false;
			int ind_king = getIndiceOfPiece(getSquareOccupier((byte) 0x04),
					true);
			int ind_rook = getIndiceOfPiece(getSquareOccupier((byte) 0x07),
					true);
			map[ind_king] = map[ind_king].move((byte) (2 * RIGHT_MOVE));
			map[ind_rook] = map[ind_rook].move((byte) (2 * LEFT_MOVE));
			return new Position((byte) 0, (byte) -1, cstl_rights, false, map,
					black_map);
		} else if (modifier == 2) {
			cstl_rights[1] = false;
			cstl_rights[3] = false;
			int ind_king = getIndiceOfPiece(getSquareOccupier((byte) 0x74),
					false);
			int ind_rook = getIndiceOfPiece(getSquareOccupier((byte) 0x77),
					false);
			map[ind_king] = map[ind_king].move((byte) (2 * RIGHT_MOVE));
			map[ind_rook] = map[ind_rook].move((byte) (2 * LEFT_MOVE));
			return new Position((byte) 0, (byte) -1, cstl_rights, true,
					white_map, map);
		} else if (modifier == 3) {
			cstl_rights[0] = false;
			cstl_rights[2] = false;
			int ind_king = getIndiceOfPiece(getSquareOccupier((byte) 0x04),
					true);
			int ind_rook = getIndiceOfPiece(getSquareOccupier((byte) 0x00),
					true);
			...
			int ind_king = getIndiceOfPiece(getSquareOccupier((byte) 0x74),
					false);
			int ind_rook = getIndiceOfPiece(getSquareOccupier((byte) 0x70),
					false);
			map[ind_king] = map[ind_king].move((byte) (2 * LEFT_MOVE));
			map[ind_rook] = map[ind_rook].move((byte) (3 * RIGHT_MOVE));
			return new Position((byte) 0, (byte) -1, cstl_rights, true,
			...
			Piece p = getSquareOccupier(start, is_White_to_Move), o = getSquareOccupier(
					end, !is_White_to_Move);
			int ind_PieceToUpdate = getIndiceOfPiece(p, is_White_to_Move), ind_CapturedPiece = getIndiceOfPiece(
					o, !is_White_to_Move);
			boolean addply = true;
			if (p.getType() == Piece.KING) {
				...
				if (start == (is_White_to_Move ? 0x00 : 0x70))
					cstl_rights[is_White_to_Move ? 2 : 3] = false;
				else if (start == (is_White_to_Move ? 0x07 : 0x77))
					cstl_rights[is_White_to_Move ? 0 : 1] = false;
			} else if (p.getType() == Piece.PAWN) {
				if (start >> 4 == 0x06 && end >> 4 == 0x04)
					epsq = (byte) (start + DOWN_MOVE);
				else if (start >> 4 == 0x01 && end >> 4 == 0x03)
					epsq = (byte) (start + UP_MOVE);
				addply = false;
			}
	...
	public int getResult() {
		...
		for (Piece b_p : black_map) {
			if (b_p.getType() == Piece.KING)
				draw = true;
			else if (b_p.getType() == Piece.BISHOP) {
				if (sq_col == -1)
					sq_col = ((b_p.getPosition() >> 4) + (b_p.getPosition() & 0x7)) % 2;
